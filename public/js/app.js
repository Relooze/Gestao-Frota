...
function modalUsuario(vs){
 ...
 <label>Perfil<select name="perfil" id="perfilUser" required><option value="motorista">Motorista</option><option value="supervisor">Supervisor</option><option value="admin">Administrador</option><option value="loja">Loja / Auditor</option></select></label>
 <label id="lojaUserWrap" style="display:none">Loja vinculada<select name="loja_vinculada" id="lojaUser"><option value="">Selecione a loja</option>${AUDIT_LOJAS.map(l=>`<option value="${l}">${l}</option>`).join("")}</select></label>
...
async function loadCombustivel(){
  ...
  try{
    ...
    $('#content').innerHTML=`<section class="panel"><div class="os-head"><div><h2>⛽ Controle de Combustível</h2><p>Clique no veículo para visualizar o consumo e o histórico individual.</p></div>${['admin','supervisor'].includes(String(user?.perfil||'').toLowerCase())?'<button class="secondary" id="fuelRecalcular">🔄 Atualizar média</button>':''}</div><div class="vehicle-chips"><button type="button" class="vehicle-chip active" data-fuel-id="">TODOS<small>${rows.length} registros</small></button>${chips}</div></section><div id="fuelResumo" style="margin-top:16px"></div><section class="panel" style="margin-top:16px"><div class="table-wrap"><table><thead><tr><th>Data</th><th>Veículo</th><th>Placa</th><th>KM anterior</th><th>KM atual</th><th>KM rodados</th><th>Litros</th><th>Média</th><th>Ação</th></tr></thead><tbody id="fuelBody"></tbody></table></div></section>`;
...
async function loadAuditoriaLoja(){
 let loja=(String(user?.perfil||'').toLowerCase()==='loja' ? user?.loja_vinculada : localStorage.getItem('auditoria_loja')); if(!loja)return selecionarLojaAuditoria();
 $('#pageTitle').textContent='Auditoria de Loja'; let idx=0;
 const secoes=AUDIT_SECOES.map(([s,itens])=>`<section class="panel audit-section"><h3>${s}</h3>${itens.map(item=>{const i=idx++;return `<div class="audit-item" data-ai="${i}" data-setor="${escapeHtml(s)}" data-item="${escapeHtml(item)}"><div class="audit-question"><b>${escapeHtml(item)}</b><div class="audit-score">${[1,2,3,4,5].map(n=>`<label><input type="radio" name="nota_${i}" value="${n}"><span>${n}</span></label>`).join('')}</div><small class="audit-rating">Selecione a nota</small></div><label>Observação / ação corretiva<textarea class="audit-obs" placeholder="Descreva a situação observada e a ação necessária, se houver."></textarea></label><label class="audit-photo">📷 Foto (obrigatória somente para nota 1 ou 2)<input type="file" accept="image/*" capture="environment" class="audit-file"><span class="audit-photo-status">Nenhuma foto anexada</span></label></div>`}).join('')}</section>`).join('');
 $('#content').innerHTML=`<section class="panel"><div class="os-head"><div><h2>🏪 ${loja}</h2><p>Auditor: <b>${escapeHtml(user?.nome||'')}</b> • Data: ${new Date().toLocaleDateString('pt-BR')}</p></div>${String(user?.perfil||'').toLowerCase()==='loja'?'':'<button class="secondary" id="trocarLojaAudit">Trocar loja</button>'}</div><div class="audit-legend">5 Excelente • 4 Bom • 3 Atenção • 2 Não conforme • 1 Crítico</div><label>Responsável da loja<input id="auditResponsavel" placeholder="Nome do responsável que acompanhou a auditoria"></label></section>${secoes}<section class="panel"><button class="primary" id="finalizarAudit">✅ Finalizar e enviar auditoria</button></section>`;
 if($('#trocarLojaAudit')) $('#trocarLojaAudit').onclick=()=>{localStorage.removeItem('auditoria_loja');selecionarLojaAuditoria()};
 document.querySelectorAll('.audit-item').forEach(el=>{el.querySelectorAll('input[type=radio]').forEach(r=>r.onchange=()=>{const nota=Number(r.value);el.querySelector('.audit-rating').textContent=auditClass(nota);const lab=el.querySelector('.audit-photo');if(lab)lab.childNodes[0].textContent=nota<=2?'📷 Foto obrigatória para não conformidade ':'📷 Foto opcional '});el.querySelector('.audit-file').onchange=async e=>{const f=e.target.files[0];if(!f)return;el.dataset.foto=await reduzirFoto(f);el.querySelector('.audit-photo-status').textContent='✅ Foto anexada'}});
 $('#finalizarAudit').onclick=async()=>{const els=[...document.querySelectorAll('.audit-item')],itens=[];for(const el of els){const n=el.querySelector('input[type=radio]:checked');if(!n){alert('Preencha a nota de todos os itens.');el.scrollIntoView({behavior:'smooth'});return}const nota=Number(n.value),obs=el.querySelector('.audit-obs').value.trim();if(nota<=2&&!el.dataset.foto){alert('Itens com nota 1 ou 2 precisam de foto da não conformidade.');el.scrollIntoView({behavior:'smooth'});return}if(nota<=2&&!obs){alert('Itens com nota 1 ou 2 precisam de observação.');el.scrollIntoView({behavior:'smooth'});return}itens.push({setor:el.dataset.setor,item:el.dataset.item,nota,classificacao:auditClass(nota),observacao:obs,foto:el.dataset.foto||null})}if(!confirm(`Finalizar auditoria da ${loja}?`))return;try{const r=await api('/api/auditoria/checklists',{method:'POST',body:JSON.stringify({loja,responsavel:$('#auditResponsavel').value.trim(),itens})});alert(`Auditoria salva. Nota geral: ${Number(r.auditoria.nota_geral).toFixed(2)}`);loadAuditoriaLoja()}catch(e){alert(e.message)}};
}
